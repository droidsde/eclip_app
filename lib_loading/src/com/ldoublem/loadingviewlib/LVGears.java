...
public class LVGears extends View {
    ...
    private void drawWheelBig(Canvas canvas) {
        ...
        for (int i = 0; i < 360; i = i + mWheelBigSpace) {
            int angle = (int) (mAnimatedValue * mWheelBigSpace + i);//clockwise é¡ºæ—¶é’ˆ
    ...
    private void drawWheelSmall(Canvas canvas) {
        ...
        for (int i = 0; i < 360; i = i + mWheelSmallSpace) {
            int angle = (int) (360 - mAnimatedValue * mWheelBigSpace + i);//anticlockwise é€†æ—¶é’ˆ
    ...
    private ValueAnimator startViewAnim(int startF, final int endF, long time) {
        ...
        valueAnimator.setRepeatCount(ValueAnimator.INFINITE);//æ— é™�å¾ªçŽ¯
        valueAnimator.setRepeatMode(ValueAnimator.RESTART);
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                mAnimatedValue = ((Float) (valueAnimator.getAnimatedValue())).floatValue();
