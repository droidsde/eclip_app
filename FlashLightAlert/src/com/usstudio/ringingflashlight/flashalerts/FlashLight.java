...
public class FlashLight {
	...
	private final static String TAG = FlashLight.class.getSimpleName();
	public static boolean gotCam = false;
	private static Camera cam;
	private static int running;
	//	private static SurfaceView preview;
	private static FlashlightCaller cf;
	//	private static SurfaceHolder mHolder;
	private static Camera.Parameters pon, poff;
	private static LinearLayout orientationChanger;


	public FlashLight(FlashlightCaller cf) {
		FlashLight.cf = cf;
		if (!gotCam) {
			try {
				cam = Camera.open();
				if (FlashlightCaller.LOG) Log.d(TAG, "Constructor: Camera opened successfully");
				gotCam = true;
				...
				if (FlashlightCaller.LOG) Log.d(TAG, "Constructor: Unable to open the camera. Is already Opened??");
				gotCam = false;
				return;
	...
	public static void incRunning() {
		...
		FlashLight.running = FlashLight.running + 1;
		if (FlashlightCaller.LOG) Log.d(TAG, "running: " + running);
	}
	...
	public static void releaseCam() {
		if (FlashlightCaller.LOG) Log.d(TAG, "releaseCam");
		if (cf.getType() == FlashlightCaller.TYPE_ALTERNATIVE || cf.getType() == FlashlightCaller.TYPE_ALTERNATIVE_2) {
			try {
				orientationChanger.setVisibility(View.GONE);
			} catch (Exception e) {
				e.printStackTrace();
				if (FlashlightCaller.LOG) Log.d(TAG, "Failed to destroy SurfaceView");
			}
		}
		if (gotCam) {
			try {
				...
				cam.release();
				gotCam = false;
				if (FlashlightCaller.LOG) Log.d(TAG, "Cam released");
			} catch (Exception e) {
				e.printStackTrace();
				if (FlashlightCaller.LOG) Log.d(TAG, "failed to release cam");
			}
		}
	}
	...
	private void loadParameters() {
		if (cf.getType() == FlashlightCaller.TYPE_ALTERNATIVE || cf.getType() == FlashlightCaller.TYPE_ALTERNATIVE_2) {
			...
			if (cf.getType() == FlashlightCaller.TYPE_ALTERNATIVE) {
				layoutParams.setMargins(1000, 1000, 1000, 1000);
			} else if (cf.getType() == FlashlightCaller.TYPE_ALTERNATIVE_2) {
			...
			SurfaceKamera preview = new SurfaceKamera(cf.getApplicationContext(), cam);
			preview.setLayoutParams(layoutParams);
		...
		try {
			...
			if (FlashlightCaller.LOG) Log.d(TAG, "unable to get parameters");
			e.printStackTrace();
	...
	public void enableFlash(long onMillis, long offMillis) {
		...
		if (!gotCam) {
			try {
				cam = Camera.open();
				if (FlashlightCaller.LOG) Log.d(TAG, "enableFlash: Camera opened successfully");
				gotCam = true;
				...
				if (FlashlightCaller.LOG) Log.d(TAG, "enableFlash: Unable to open the camera. Is already Opened??");
				gotCam = false;
				try {
					Thread.sleep(onMillis + offMillis);
				} catch (InterruptedException e1) {
		...
		try {
			...
			e.printStackTrace();
			if (FlashlightCaller.LOG) Log.d(TAG, "failed to set parameters");
			//BugSenseHandler.sendException(e);
		}
	...
	public void disableFlash() {
		if (FlashlightCaller.LOG) Log.d(TAG, "disableFlash");
