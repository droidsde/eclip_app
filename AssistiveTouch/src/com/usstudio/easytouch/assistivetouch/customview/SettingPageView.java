...
public class SettingPageView implements OnClickListener {
	private View view;
...
//	private TextView textView5;
	...
	public SettingPageView(View view, Context context) {
		this.view = view;
...
//		taskLayout5.setOnClickListener(this);
		...
		for (int i = 0; i < data.length; i++) {
			data[i] = SharedPreference.readSettingPage(context, i + 1).split("[.]");
		}
...
//			break;
	...
	private void showDialog(final int settingPage, final ImageView taskImage, final TextView textView) {
		ChooseTaskDialog dialog = new ChooseTaskDialog(context, settingPage, 2);
		dialog.setOnDismissListener(new OnDismissListener() {

			@Override
			public void onDismiss(DialogInterface dialog) {
				data[settingPage - 1] = SharedPreference.readSettingPage(context, settingPage).split("[.]");
				initLayout(taskImage, textView, data[settingPage - 1][0], data[settingPage - 1][1]);
			}
		});
